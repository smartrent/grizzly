defmodule Grizzly.ZWave.Commands.NodeAdd do
  ...
  def decode_params(<<seq_number, _reserved, mode_byte, tx_opt_byte>>) do
    [mode: decode_mode(mode_byte), seq_number: seq_number, tx_opt: decode_tx_opt(tx_opt_byte)]
  end
  ...
  def decode_mode(0x01), do: :node_add_any
  def decode_mode(0x05), do: :node_add_stop
  def decode_mode(0x07), do: :node_add_s2_any

  @spec decode_tx_opt(byte()) :: tx_opt()
  def decode_tx_opt(0x00), do: :null
  def decode_tx_opt(0x02), do: :low_power
  def decode_tx_opt(0x20), do: :explore
end
