defmodule Grizzly.ZWave.Commands.ApplicationBusy do
  ...
  Params:
    ...
    * `:status` - The status field can have the following values: :try_again_later, :try_again_after_wait, :request_queued (required)

    * `:wait_time` - The wait time field indicates the number of seconds a node should wait before retrying the request (required)
