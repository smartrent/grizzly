defmodule Grizzly.ZWave.IconType do
  ...
  @inverse_mappings Map.new(@mappings, fn {k, v} -> {v, k} end)

  @doc """
  Get the icon type's name from a 16-bit integer value
  """
  @spec to_name(value()) :: {:ok, name()} | {:error, :unknown_icon_type}
  def to_name(value) do
    case Map.fetch(@mappings, value) do
      {:ok, name} -> {:ok, name}
      :error -> {:error, :unknown_icon_type}
    end
  end
  ...
  def to_value(name) do
    case Map.fetch(@inverse_mappings, name) do
      {:ok, value} -> {:ok, value}
      :error -> {:error, :unknown_icon_type}
    end
  end
