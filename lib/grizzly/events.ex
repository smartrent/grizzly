defmodule Grizzly.Events do
  ...
  * `:firehose` - Normally, subscribers will only receive unsolicited commands
  ...
  def subscribe(node_id, opts) when is_node_id(node_id) do
    subscribe({:node, node_id}, opts)
  end

  def subscribe(event, _opts) when is_event(event) do
    _ = Registry.register(__MODULE__, event, [])
    :ok
  end

  def subscribe(subject, opts) do
    _ = Registry.register(__MODULE__, subject, Map.new(opts))
    :ok
  end
  ...
  defp match_spec_guards_for_unsolicited(node_id, command_name) do
    command_name_match = {:==, :"$1", {:const, command_name}}
    node_id_match = {:==, :"$1", {{:node, {:const, node_id}}}}
  ...
  defp match_spec_guards_for_firehose(node_id, command_name) do
    ...
    # erlang's map_get's arguments opposite elixir's map_get!
