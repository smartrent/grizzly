defmodule Grizzly.CommandClass.MultiChannelAssociation.Remove do
  @moduledoc """
  ...
  def encode(
    ...
    with {:ok, _encoded} <-
      ...
      {:ok, encoded_endpoints} = MultiChannelAssociation.encode_endpoints(endpoints)

      binary =
        Packet.header(seq_number) <>
          <<0x8E, 0x04, group>> <>
          encoded_nodes <> <<MultiChannelAssociation.marker()>> <> encoded_endpoints
