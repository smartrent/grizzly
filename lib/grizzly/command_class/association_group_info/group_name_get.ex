defmodule Grizzly.CommandClass.AssociationGroupInfo.GroupNameGet do
  @moduledoc """
  ...
  def encode(%__MODULE__{seq_number: seq_number, group: group} = command) do
    with {:ok, _encoded} <-
      ...
      binary = Packet.header(seq_number) <> <<0x59, 0x01, group>>
      {:ok, binary}
    end
  end
  ...
  @spec handle_response(t, Packet.t()) ::
          ...
          | {:done, {:done, AssociationGroupInfo.group_name_report()}}
          | {:retry, t()}
          | {:queued, t()}
