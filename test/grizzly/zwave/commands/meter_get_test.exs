defmodule Grizzly.ZWave.Commands.MeterGetTest do
  ...
  test "encodes params correctly" do
    ...
    assert MeterGet.encode_params(nil, command) == <<>>

    # v2-3
    params = [scale: :kwh]
    {:ok, command} = Commands.create(:meter_get, params)
    assert MeterGet.encode_params(nil, command) == <<0::2, 0::3, 0::3>>

    # v4-6
    params = [scale: :kwh, rate_type: :default]
    {:ok, command} = Commands.create(:meter_get, params)
    assert MeterGet.encode_params(nil, command) == <<0::2, 0::3, 0::3, 0>>

    # v4-6
    params = [scale: :kwh, rate_type: :import]
    {:ok, command} = Commands.create(:meter_get, params)
    assert MeterGet.encode_params(nil, command) == <<1::2, 0::3, 0::3, 0>>

    # v4-6
    params = [scale: :kwh, rate_type: :import_export]
    {:ok, command} = Commands.create(:meter_get, params)
    assert MeterGet.encode_params(nil, command) == <<3::2, 0::3, 0::3, 0>>

    # v4-6
    params = [scale: :pulse_count, rate_type: :import_export]
    {:ok, command} = Commands.create(:meter_get, params)
    assert MeterGet.encode_params(nil, command) == <<3::2, 3::3, 0::3, 0>>

    # v4-6
    params = [scale: :kvarh, rate_type: :import_export]
    {:ok, command} = Commands.create(:meter_get, params)
    assert MeterGet.encode_params(nil, command) == <<3::2, 7::3, 0::3, 1>>

    # v4-6
    params = [scale: :kvar]
    {:ok, command} = Commands.create(:meter_get, params)
    assert MeterGet.encode_params(nil, command) == <<0::2, 7::3, 0::3, 0>>
  ...
  test "decodes params correctly" do
    # v1
    binary = <<>>
    assert {:ok, params} = MeterGet.decode_params(nil, binary)
