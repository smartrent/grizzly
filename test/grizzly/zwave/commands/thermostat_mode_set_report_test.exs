defmodule Grizzly.ZWave.Commands.ThermostatModeSetReportTest do
  ...
  test "encodes params correctly" do
    ...
    assert expected_binary == ThermostatModeSetReport.encode_params(command)

    assert_raise KeyError, fn ->
      params = [mode: :manufacturer_specific]
      {:ok, cmd} = Commands.create(:thermostat_mode_set, params)
      ThermostatModeSetReport.encode_params(cmd)
    end
    ...
    {:ok, cmd} = Commands.create(:thermostat_mode_set, params)
    assert <<0xFF, "1234567">> = ThermostatModeSetReport.encode_params(cmd)
  end

  test "decodes params correctly" do
    binary_params = <<0x00::3, 0x02::5>>
    {:ok, params} = ThermostatModeSetReport.decode_params(binary_params)
    assert Keyword.get(params, :mode) == :cool

    assert {:ok, params} = ThermostatModeSetReport.decode_params(<<0xFF, "1234567">>)
