defmodule Grizzly.ZWave.CommandClasses.SecurityTest do
  ...
  test "derives correct authentication and encryption keys from the network key" do
    ...
    assert expected_auth_key == Security.authentication_key(@network_key)
    assert expected_enc_key == Security.encryption_key(@network_key)
  end
  ...
  test "encrypts and decrypts a payload correctly" do
    ...
    assert decrypted_payload ==
             Security.decrypt(@network_key, sender_nonce, receiver_nonce, encrypted_payload)

    assert encrypted_payload ==
             Security.encrypt(@network_key, sender_nonce, receiver_nonce, decrypted_payload)
  end
end
