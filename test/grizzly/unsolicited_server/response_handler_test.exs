defmodule Grizzly.UnsolicitedServer.ResponseHandlerTest do
  ...
  test "handle non-extra command" do
    ...
    assert [:ack, {:notify, report}] == ResponseHandler.handle_response(6, response)
  end
  ...
  test "handle association specific group get" do
    ...
    assert [:ack, {:send, asgr}] = ResponseHandler.handle_response(6, response)
  ...
  test "handle association get for known support grouping identifier", %{assoc_server: server} do
    ...
    assert [:ack, {:send, assoc_report}] =
             ResponseHandler.handle_response(6, response, associations_server: server)
  ...
  test "handle association set", %{assoc_server: server} do
    ...
    assert [:ack] == ResponseHandler.handle_response(6, response, associations_server: server)
  end

  test "handle association groupings get" do
    {:ok, agg} = AssociationGroupingsGet.new()
    assert [:ack, {:send, agr}] = ResponseHandler.handle_response(6, make_response(agg))
  ...
  describe "association group" do
    test "name get - known group (lifeline)" do
      ...
      assert [:ack, {:send, agnr}] = ResponseHandler.handle_response(6, make_response(agng))
    ...
    test "info get" do
      ...
      assert [:ack, {:send, agir}] = ResponseHandler.handle_response(6, make_response(agig))
    ...
    test "command list get" do
      ...
      assert [:ack, {:send, agclr}] = ResponseHandler.handle_response(6, make_response(agclg))
  ...
  describe "supervision" do
    test "get" do
      ...
      assert [:ack, {:notify, notify_command}, {:send, report}] =
               ResponseHandler.handle_response(6, make_response(sup_get))
  ...
  describe "multi channel association" do
    test "groupings get" do
      ...
      assert [:ack, {:send, mcagr}] = ResponseHandler.handle_response(6, make_response(mcagg))
    ...
    test "get", %{multi_channel_server: server} do
      ...
      assert [:ack, {:send, mcar}] =
               ResponseHandler.handle_response(6, make_response(mcag),
    ...
    test "set", %{multi_channel_server: server} do
      ...
      assert [:ack] =
               ResponseHandler.handle_response(6, make_response(mcas),
                 associations_server: server
               )
    end
    ...
    test "remove", %{multi_channel_server: server} do
      ...
      assert [:ack] =
               ResponseHandler.handle_response(6, make_response(mcar),
                 associations_server: server
               )
    end
  end
  ...
  test "version get query" do
    ...
    assert [:ack, {:send, version_report}] =
             ResponseHandler.handle_response(6, make_response(version_get))
