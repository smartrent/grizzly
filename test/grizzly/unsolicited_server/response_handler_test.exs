defmodule Grizzly.UnsolicitedServer.ResponseHandlerTest do
  ...
  test "handle non-extra command" do
    ...
    assert [:ack, {:notify, report}] == ResponseHandler.handle_response(response)
  end
  ...
  test "handle association specific group get" do
    ...
    assert [:ack, {:send, asgr}] = ResponseHandler.handle_response(response)
  ...
  test "handle association set", %{assoc_server: server} do
    ...
    assert [:ack] == ResponseHandler.handle_response(response, associations_server: server)
  end

  test "handle association groupings get" do
    {:ok, agg} = AssociationGroupingsGet.new()
    assert [:ack, {:send, agr}] = ResponseHandler.handle_response(make_response(agg))
  ...
  describe "association group" do
    test "name get - known group (lifeline)" do
      ...
      assert [:ack, {:send, agnr}] = ResponseHandler.handle_response(make_response(agng))
    ...
    test "info get" do
      ...
      assert [:ack, {:send, agir}] = ResponseHandler.handle_response(make_response(agig))
    ...
    test "command list get" do
      ...
      assert [:ack, {:send, agclr}] = ResponseHandler.handle_response(make_response(agclg))
  ...
  describe "multi channel association" do
    test "groupings get" do
      ...
      assert [:ack, {:send, mcagr}] = ResponseHandler.handle_response(make_response(mcagg))
  ...
  test "version get query" do
    ...
    assert [:ack, {:send, version_report}] =
             ResponseHandler.handle_response(make_response(version_get))
