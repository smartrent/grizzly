defmodule Grizzly.CommandClass.ThermostatFanState.Get.Test do
  ...
  describe "implements the Grizzly.Command behaviour" do
    ...
    test "encodes correctly" do
      ...
      binary = <<35, 2, 128, 208, 8, 0, 0, 3, 2, 0, 0x45, 0x02>>
    ...
    test "handles thermostat fan state report" do
      report = %{command_class: :thermostat_fan_state, command: :report, value: :running}
      {:ok, command} = Get.init(seq_number: 0x10)
      packet = Packet.new(body: report)

      assert {:done, {:ok, :running}} == Get.handle_response(command, packet)
    end
