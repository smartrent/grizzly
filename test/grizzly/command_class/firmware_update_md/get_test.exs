defmodule Grizzly.CommandClass.FirmwareUpdateMD.GetTest do
  use ExUnit.Case, async: true

  alias Grizzly.Packet
  alias Grizzly.CommandClass.FirmwareUpdateMD.Get
  ...
  describe "implements Grizzly.Command behaviour" do
    ...
    test "encodes correctly" do
      ...
      binary = <<35, 2, 128, 208, 8, 0, 0, 3, 2, 0, 0x7A, 0x01>>
    ...
    test "handles basic report responses" do
      ...
      body = %{command_class: FirmwareUpdateMD, command: :report, value: report}
      {:ok, command} = Get.init([])
      packet = Packet.new(body: body)

      assert {:done, {:ok, report}} == Get.handle_response(command, packet)
    end
