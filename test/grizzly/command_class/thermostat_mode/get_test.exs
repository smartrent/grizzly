defmodule Grizzly.CommandClass.ThermostatMode.Get.Test do
  use ExUnit.Case, async: true

  alias Grizzly.Packet
  alias Grizzly.CommandClass.ThermostatMode.Get
  ...
  describe "implements the Grizzly.Command behaviour" do
    ...
    test "encodes correctly" do
      ...
      binary = <<35, 2, 128, 208, 8, 0, 0, 3, 2, 0, 0x40, 0x02>>
    ...
    test "handles thermostat mode report" do
      report = %{command_class: :thermostat_mode, command: :report, value: :low}
      {:ok, command} = Get.init([])
      packet = Packet.new(body: report)

      assert {:done, {:ok, :low}} == Get.handle_response(command, packet)
    end
