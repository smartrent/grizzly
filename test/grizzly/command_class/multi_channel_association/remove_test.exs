defmodule Grizzly.CommandClass.MultiChannelAssociation.Remove.Test do
  use ExUnit.Case, async: true

  alias Grizzly.Packet
  alias Grizzly.CommandClass.MultiChannelAssociation.Remove
  ...
  describe "implements the Grizzly command behaviour" do
    test "initializes the command state" do
      ...
      {:ok, command} =
        Remove.init(group: 2, nodes: [2, 3], endpoints: endpoints, seq_number: 0x06)

      assert %Remove{group: 2, nodes: [2, 3], endpoints: endpoints, seq_number: 0x06} == command
    end
    ...
    test "encodes correctly" do
      ...
      {:ok, command} =
        Remove.init(group: 2, nodes: [2, 3], endpoints: endpoints, seq_number: 0x06, retries: 0)

      binary =
        <<35, 2, 128, 208, 6, 0, 0, 3, 2, 0, 0x8E, 0x04, 0x02, 0x02, 0x03, 0x00, 0x02, 0x02, 0x03,
          0x03, 0x03, 0x04>>

      assert {:ok, binary} == Remove.encode(command)
    end
    ...
    test "handles an ack response" do
      ...
      {:ok, command} =
        Remove.init(group: 2, nodes: [2, 3], endpoints: endpoints, seq_number: 0x01)
    ...
    test "handles a nack response" do
      ...
      {:ok, command} =
        Remove.init(group: 2, nodes: [2, 3], endpoints: endpoints, seq_number: 0x01, retries: 0)
    ...
    test "handles retries" do
      ...
      assert {:retry, _command} = Remove.handle_response(command, packet)
    end
    ...
    test "handles responses" do
      ...
      assert {:continue, _} = Remove.handle_response(command, %{})
    end
  end
end
