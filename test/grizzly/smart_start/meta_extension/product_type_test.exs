defmodule Grizzly.SmartStart.MetaExtension.ProductTypeTest do
  ...
  test "when generic type is not supported" do
    assert {:error, :invalid_generic_device_class} ==
             ProductType.new(
               :fake_gen_device_class,
               :routing_sensor_binary,
               :icon_type_generic_sensor_notification
             )
  end
  ...
  test "when specific type is not supported" do
    assert {:error, :invalid_specific_device_class} ==
             ProductType.new(
               :sensor_binary,
               :not_a_sensor_binary,
               :icon_type_generic_sensor_notification
  ...
  test "when icon type is not supported" do
    assert {:error, :unknown_icon_type} ==
             ProductType.new(
               :sensor_binary,
               :routing_sensor_binary,
               :icon_type_generic_robot_dog_installer
             )
  end
